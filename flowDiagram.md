...
flowchart TD
    ...
    MB -->|unload_others before turn| S4
    MB -->|serve| EXT3
    ...
    S2 -->|chat| S4
    S4 -->|POST /api/chat non-stream| EXT2
...
- `modules/ollama.py` — shared Ollama helpers: `chat(model_id, messages, options)` posts non-streaming to `/api/chat` and raises `RuntimeError` on HTTP errors or Ollama-level `{"error": ...}` responses (callers in `main.py` map that to HTTP 502); `unload_others(model_id)` unloads every other resident model before a turn (Ollama overcommits unified GPU memory, and a second resident model silently corrupts generation). The streaming turn generation in `main.py` keeps its own inline client.
