...
- `modules/ollama.py` — shared non-streaming Ollama chat helper used by all business-logic modules: `chat(model_id, messages, options)` posts to `/api/chat`, raises `RuntimeError` on HTTP errors or Ollama-level `{"error": ...}` responses (callers in `main.py` map that to HTTP 502). The streaming turn generation in `main.py` keeps its own inline client.
