...
- `modules/describe.py` — scene description business logic (no FastAPI code): `generate_description` asks Ollama for a detailed visual snapshot of the current scene (camera framing, characters, clothing, poses, setting, lighting, plus a booru-style tag line) for text-to-image models; prompt: `describePrompt`, generation parameters: `describeGeneration` (both in config.json). Called by the `POST /api/games/{id}/describe` route in `main.py` (Describe button next to Continue). The result is ephemeral — nothing is stored.
...
- The `scenarios` DB table mirrors the [Character Card V2 spec](https://github.com/malfoyslastname/character-card-spec-v2/blob/main/spec_v2.md) (`name`, `description`, `personality`, `scenario`, `first_mes`, `mes_example`, `system_prompt`, `post_history_instructions`, `alternate_greetings`, `tags`, …). The games table holds one merged `system_prompt` (no more `custom_prompt`).
- `GET /api/games/{id}/scenario` exports a full `chara_card_v2` JSON document (world_cards → `character_book.entries`, character + icon → `data.extensions.storytelling`).
- `POST /api/scenarios/import` accepts a `chara_card_v2` JSON document, validates `spec`/`data.name`/character_book entries, derives the file slug from `data.name`, writes `scenarios/<slug>.json` and registers it in `scenarios/index.json`. V3 cards are rejected.
- Prompt assembly happens entirely in the frontend (`buildMessages` in game.js, mirrored by the tester's `MessageBuilder`); the backend turn route just relays the prebuilt `messages` array. Card macros `{{char}}`/`{{user}}`/`{{original}}` are replaced at build time.
